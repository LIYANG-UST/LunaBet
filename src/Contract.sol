...
/// @notice refs:
///         - https://twitter.com/AlgodTrading/status/1∏503103705939423234
...
contract LunaBet is Initializable {
    ...
    uint256 public constant betStartTimestamp = 1647243834;
    uint256 public betEndTimestamp; // initialized to be 7 days after contract deployment

    mapping(address => uint256) public doAmts; // mapping from user to bet amounts on Do's side
    mapping(address => uint256) public algodAmts; // mapping from user to bet amounts on Algod's side
    uint256 public doTotalAmt; // Total bet amount on Do's side
    uint256 public algodTotalAmt; // Total bet amount on Algod's side
    bool public isVoid; // whether the bet is void.
    Side public winner; // The bet winner.
    uint256 public winningMultiplier; // in 1e18.
    ...
    function betDo(uint256 _amt) external {
    ...
    /// @param _amt The amount of USDT to bet.
    /// @notice Must be called before the betting ends.
    function betAlgod(uint256 _amt) external {
        require(block.timestamp <= betEndTimestamp, "bet already ended");
        require(_amt > 0, "!_amt");
        usdt.safeTransferFrom(msg.sender, address(this), _amt);
    ...
    function announceWinner(Side _winner) external {
        ...
        uint256 totalAmts = doTotalAmt + algodTotalAmt;
        if (_winner == Side.DO) {
            winningMultiplier = doTotalAmt > 0
                ? (totalAmts * 1e18) / doTotalAmt
                : 0;
        } else {
            winningMultiplier = algodTotalAmt > 0
                ? (totalAmts * 1e18) / algodTotalAmt
                : 0;
        }
    }
    ...
    function claimWinningBet() external {
        ...
        uint256 claimAmt = (depositAmt * winningMultiplier) / 1e18;
    ...
    function withdrawVoidedBet() external {
        ...
        uint256 withdrawAmt = algodAmts[msg.sender] + doAmts[msg.sender];
    ...
    function rug(uint256 _amt) external {
        require(msg.sender == cobie, "!cobie");
